...
def extract_mpileup_info(input_file, output_file):
    ...
    mpileup_data = pd.read_csv(input_file, sep='\t', header=None, names=col_names,skiprows=skip_count)

   # Extract the DP value from the Info column
    mpileup_data['DP'] = mpileup_data['INFO'].str.extract(r'DP=(\d+)')
    
    # Convert the DP column to numeric, setting errors to 'coerce' which replaces non-convertible values with NaN
